...
function initAutocomplete() {
	...
	var	lat_event  = document.getElementsByName('latitude_event')[0].value ;
    var long_event = document.getElementsByName('longitude_event')[0].value ;
	var lat_ev		= parseFloat(lat_event);
	var long_ev 	= parseFloat(long_event);
	if (lat_event!='' && long_event!=''){
			var map        = new google.maps.Map(document.getElementById('map'), {
				center: {lat: lat_ev, lng: long_ev},
				zoom: 17,
				mapTypeId: 'roadmap'
			});
			var uluru = {lat: lat_ev, lng: long_ev};
			mark(map,uluru)
		} else {
			var map        = new google.maps.Map(document.getElementById('map'), {
    ...
    function clearOldDetail() {
        document.getElementsByName('latitude_event')[0].value = '';
        document.getElementsByName('longitude_event')[0].value = '';
    ...
    function getPosition(location, marker) {
        ...
        document.getElementsByName('latitude_event')[0].value = location.lat();
        document.getElementsByName('longitude_event')[0].value = location.lng();
    }
