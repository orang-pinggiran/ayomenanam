...
function initAutocomplete() {
	...
	var	lat_map  = document.getElementsByName('latitude_map')[0].value ;
    var long_map = document.getElementsByName('longitude_map')[0].value ;
	var lat_maps		= parseFloat(lat_map);
	var long_maps 	= parseFloat(long_map);
	if (lat_map!='' && long_map!=''){
			var map        = new google.maps.Map(document.getElementById('map'), {
				center: {lat: lat_maps, lng: long_maps},
				zoom: 17,
				mapTypeId: 'roadmap'
			});
			var uluru = {lat: lat_maps, lng: long_maps};
			mark(map,uluru)
		} else {
			var map        = new google.maps.Map(document.getElementById('map'), {
    ...
    function clearOldDetail() {
        document.getElementsByName('latitude_map')[0].value = '';
        document.getElementsByName('longitude_map')[0].value = '';
    ...
    function getPosition(location, marker) {
        ...
        document.getElementsByName('latitude_map')[0].value = location.lat();
        document.getElementsByName('longitude_map')[0].value = location.lng();
    }
