...
class Mails extends CI_Controller {
	...
	public function konfirmasi_registrasi() {
		//$user_id                    = $this->input->post('id_user');
		//$registered_time            = $this->input->post('registered_time');
		//$full_name                  = $this->input->post('full_name');
		//$id_pengguna = $this->input->post('id_pengguna');
		$id_pengguna 		= $this->input->post('id_pengguna');
		$data_pengguna 		= $this->m_pengguna->get_data($id_pengguna)->row_array();
		$registered_time 	= $data_pengguna['waktu_registrasi'];
		$user_email         = $data_pengguna['email'];
		
		// generate hash
		$key                        = parse_time($registered_time, 'sih');
		$hash_data                  = $id_pengguna.'|'.$registered_time;
		$hash                       = encrypt_id($hash_data, $key);
		
		// mail data
		$data['verifikasi_url'] = base_url('welcome/aktivasi-akun/'.$key.'/'.$hash);
		$data['nama_pengguna']  = $data_pengguna['nama'];
		
		// $mail to
		$mail_to      = $user_email;
		$mail_subject = 'Verifikasi Akun';
		$mail_message = $this->load->view('email/verifikasi-akun', $data, TRUE);
